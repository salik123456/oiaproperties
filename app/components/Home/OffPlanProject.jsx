...
const OffPlanProject = () => {
  const slides = [
    {
      ...
      image: '/offplan.png', // Replace with your image path
    ...
    },
    {
      ...
      image: '/offplan.png', // Replace with your image path
      villaTypes: ['1 BHK VILLA + MAID\'S', '4 BHK VILLA + MAID\'S', '5 BHK VILLA + MAID\'S'],
      description: 'Another amazing description for the second slide.',
  ...
  return (
    <div className=" mx-auto py-12">
      <h2 className="text-5xl font-semibold container mx-auto text-left text-dark flex items-center mb-8">
      ...
      <div className="relative py-10 px-8 bg-gray-200">
        <div className="flex justify-between items-center bg-gray-200 mb-6">
          <button onClick={prevSlide} className="text-gray-600 bg-gray-300 rounded-lg p-2 hover:text-gray-800 cursor-pointer">
            <IoIosArrowBack className="text-2xl" />
          </button>
          <h3 className="text-3xl font-semibold text-center">{slides[currentSlide].title}</h3>
          <button onClick={nextSlide} className="text-gray-600 bg-gray-300 rounded-lg p-2 hover:text-gray-800 cursor-pointer">
            <IoIosArrowForward className="text-2xl" />
          </button>
        </div>
        <div className="flex space-x-4 justify-center mb-6">
          {slides[currentSlide].villaTypes.map((type, index) => (
            <span key={index} className="text-lg font-medium text-gray-700 cursor-pointer">
              {type}
            </span>
          ))}
        </div>
        <AnimatePresence initial={false} custom={currentSlide}>
          ...
          >
            <div className="p-6 bg-white rounded-lg flex flex-col justify-center">
              <p className="text-gray-700 mb-4">{slides[currentSlide].description}</p>
              <p className="text-lg font-semibold mb-2">Starting Prices: {slides[currentSlide].startingPrice}</p>
              <p className="text-gray-700 mb-2">Number of Bedrooms: {slides[currentSlide].bedrooms}</p>
              <p className="text-gray-700 mb-4">Area Size From {slides[currentSlide].areaSize}</p>
