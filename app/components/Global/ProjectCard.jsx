...
const ProjectCard = ({ id, imgUrls, title, price, location, projects, activeProject, setActiveProject }) => {
  return (
    ...
    >
      {/* Always display only the first image */}
      <Image width={700} height={200} src={imgUrls[0]} alt={title} className="absolute w-full h-full object-cover" />
      ...
      {activeProject !== id ? (
        ...
        <div className="absolute inset-0 bg-black/50 flex flex-col justify-end p-4">
          ...
          <button className="bg-secondary text-md text-white px-6 py-2 rounded-full">Discover Projects</button>
          </div>
        
        </div>

<p className="text-lg font-semibold text-[32px] text-white mb-4 absolute right-0 top-0 p-4">{price}</p>
</>
      )}
    </motion.div>
  );
